...
def render(tables: list[Table], enums: dict[str, list[str]]) -> str:
    lines: list[str] = []
    ...
    # Views / Functions / CompositeTypes — empty placeholders
...
def main() -> int:
    ...
    try:
        ...
        conn.close()
    content = render(tables, enums)
    OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    OUT_PATH.write_text(content, encoding="utf-8")
    print(f"Wrote {OUT_PATH} ({len(tables)} tables, {len(enums)} enums)")
    return 0
